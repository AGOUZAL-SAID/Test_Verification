...
void test_schedule_light_on_specific_day_time(void) {
    ...
    TEST_ASSERT_TRUE(LightControlSpy_getLastState());
}
...
void test_schedule_light_off_specific_day_time(void) {
    ...
    TEST_ASSERT_FALSE(LightControlSpy_getLastState());
}
...
void test_weekday_schedule(void) {
    LightScheduler_init();
    int i =LightScheduler_schedule(3, WEEKDAY, 12*60, TURN_ON);
    turn_off_led_now(3);
...
void test_weekday_schedule_off(void) {
    LightScheduler_init();
    int i =LightScheduler_schedule(3, WEEKDAY, 12*60, TURN_OFF);
    turn_on_led_now(3);
    set_time(MONDAY, 12*60);
    LightScheduler_wakeup();
    TEST_ASSERT_EQUAL(-1, i);
...
int main(void) {
    test_schedule_light_on_specific_day_time();
    test_schedule_light_off_specific_day_time();
    test_everyday_schedule();
    test_weekend_schedule();
    test_remove_scheduled_event();
    test_max_events();
    test_everyday_schedule_off();
    test_weekday_schedule_off();
    test_weekend_schedule_off();
    test_scheduler_can_off_led();
    test_scheduler_can_on_led();
    test_on_invalid_led();
    test_off_invalid_led();
    test_schedule_light_on_specific_day_time_invalidLED();
    test_schedule_light_off_specific_day_time_invalidLED();
    test_schedule_light_on_multiple_event_oneLED();         
    return 0;
}